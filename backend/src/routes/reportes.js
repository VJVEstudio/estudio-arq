...
router.get('/general', async (req, res) => {
  ...
  const { rows: porProyecto } = await query(
    `SELECT p.id, p.nombre AS proyecto, c.nombre_razon_social AS cliente, p.estado,
            ...
            COALESCE((SELECT SUM(e2.monto) FROM egresos e2 WHERE e2.proyecto_id = p.id AND e2.moneda = 'ARS' AND e2.categoria != 'dibujantes' ${condE}), 0) AS egresos_ars,            COALESCE((SELECT SUM(e2.monto) FROM egresos e2 WHERE e2.proyecto_id = p.id AND e2.moneda = 'USD' AND e2.categoria != 'dibujantes' ${condE}), 0) AS egresos_usd,
